...
NAVAGENT_LABELED_DIR="${NAVAGENT_LABELED_DIR:-/root/data1/SFT-ovon/labeled}"
NAVAGENT_RAW_DIR="${NAVAGENT_RAW_DIR:-/root/data1/SFT-ovon/raw}"
NAVAGENT_QWENVL_DATA_DIR="${NAVAGENT_QWENVL_DATA_DIR:-/root/data1/SFT-ovon/qwenvl}"
...
# 训练 / 验证集切分
VAL_SPLIT_ENABLE="${VAL_SPLIT_ENABLE:-1}"
...
ESTIMATING_SUB_STRATIFY_GOAL_TYPE="${ESTIMATING_SUB_STRATIFY_GOAL_TYPE:-0}"
...
PLANNING_SUB_STRATIFY_GOAL_TYPE="${PLANNING_SUB_STRATIFY_GOAL_TYPE:-0}"
...
if [[ "${SKIP_EXPORT}" == "1" ]]; then
  ...
  CMD=(
    python "/root/NavAgent/data_gen/run_export_qwenvl.py"
  ...
  echo "导出 NavAgent Qwen-VL 数据 -> ${NAVAGENT_QWENVL_DATA_DIR}"
  "${CMD[@]}"
fi
