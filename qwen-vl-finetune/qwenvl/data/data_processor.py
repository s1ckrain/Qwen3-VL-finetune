...
def _normalize_skill_name(name: Optional[str]) -> str:
    ...
    # Strip the train/val split suffixes produced by tools/split_navagent_val.py
    # so that per-skill weights and per-skill loss accounting stay keyed on the
    # underlying skill name (e.g. estimating, planning).
...
def parse_dataset_resample_weights(spec: str) -> Dict[str, float]:
    ...
    for item in spec.split(","):
        ...
        if "=" not in piece:
            raise ValueError(
                "dataset_resample_weights must look like "
                "'observing=1,estimating=1' or 'navagent_planning=2'"
            )
        key, value = piece.split("=", 1)
        key = _normalize_skill_name(key)
        try:
            weights[key] = float(value)
        except ValueError as exc:
            raise ValueError(
                f"Invalid weight '{value}' in dataset_resample_weights"
            ) from exc
    return weights


def _lookup_resample_weight(dataset_name: str, weight_map: Dict[str, float]) -> float:
    if not weight_map:
        return 1.0
    exact = str(dataset_name).strip().lower()
    if exact in weight_map:
        return weight_map[exact]
    normalized = _normalize_skill_name(dataset_name)
    return weight_map.get(normalized, 1.0)


def _attach_dataset_metadata(annotation: Any, data_config: Dict[str, Any]) -> None:
    dataset_name = data_config.get("dataset_name", "")
    if isinstance(annotation, list):
        for sub_ann in annotation:
            _attach_dataset_metadata(sub_ann, data_config)
        return

    annotation["data_path"] = data_config["data_path"]
    annotation["dataset_name"] = dataset_name
    annotation["skill"] = _normalize_skill_name(
        annotation.get("skill")
        or annotation.get("meta", {}).get("skill")
        or dataset_name
    )


def _resample_dataset_entries(
    ...
    weight_map = parse_dataset_resample_weights(weight_spec)
    if not weight_map:
        merged: List[Dict[str, Any]] = []
        for entry in dataset_entries:
            merged.extend(entry["annotations"])
        return merged
    ...
    for entry in dataset_entries:
        ...
        weight = _lookup_resample_weight(dataset_name, weight_map)
        prepared.append((dataset_name, annotations, weight))

    positive = [
        (name, annotations, weight)
        for name, annotations, weight in prepared
        if annotations and weight > 0
    ]
    if not positive:
        raise ValueError(
            "All datasets were disabled by dataset_resample_weights."
        )

    base_count = max(len(annotations) / weight for _, annotations, weight in positive)

    merged: List[Dict[str, Any]] = []
    for dataset_name, annotations, weight in prepared:
        if not annotations:
            rank0_print(f"dataset {dataset_name} is empty; skipping")
            continue
        if weight <= 0:
            rank0_print(f"dataset {dataset_name} disabled by weight={weight}")
            continue

        target_count = max(1, int(round(base_count * weight)))
        if target_count < len(annotations):
            sampled = random.sample(annotations, target_count)
        elif target_count > len(annotations):
            sampled = list(annotations) + random.choices(
                annotations, k=target_count - len(annotations)
            )
        else:
            sampled = list(annotations)

        rank0_print(
            f"resampled dataset {dataset_name}: "
            f"original={len(annotations)} target={target_count} weight={weight}"
        )
...
def preprocess_qwen_visual(
    ...
    full_result["skill"] = _normalize_skill_name(
        source.get("skill")
        or source.get("meta", {}).get("skill")
...
class LazySupervisedDataset(Dataset):
    ...
    def __init__(self, processor, data_args):
        ...
        for data in dataset_list:
            ...
            for ann in annotations:
                _attach_dataset_metadata(ann, data)
        ...
        list_data_dict = _resample_dataset_entries(
            dataset_entries,
            getattr(data_args, "dataset_resample_weights", ""),
        )
    ...
    def _get_packed_item(self, sources) -> Dict[str, torch.Tensor]:
        ...
        if isinstance(sources, list):
            ...
            skills = [d.get("skill", "unknown") for d in data_list]
            new_data_dict["skill"] = (
                skills[0] if skills and all(s == skills[0] for s in skills) else "packed_mixed"
            )
...
class DataCollatorForSupervisedDataset(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        batch["skills"] = [
            _normalize_skill_name(instance.get("skill", "unknown"))
            for instance in instances
        ]
        return batch
